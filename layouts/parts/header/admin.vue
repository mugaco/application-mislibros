<template>
  <v-app-bar
    ...
    elevate-on-scroll
  >
    <v-btn icon @click="$store.commit('mod/toggleDrawerMenu')">
      <v-icon>
        {{ $store.state.mod.drawerMenu ? "menu_open" : "menu" }}
      </v-icon>
    </v-btn>

    <v-container class="pa-0 fill-height">
      <Logo v-show="$store.state.user.logged && !$store.state.mod.drawerMenu" />
      <v-spacer />
      <div>
        <Cart />
        <Langs />
